...
extern "C" void cpy_continue(
    ...
    const std::vector<int>& dst_byte_stride,
...
PYBIND11_MODULE(cpy_continue_me, m) {
    ...
    m.def("cpy_con", [](py::buffer src,  // 不指明 dtye
                        py::buffer dst, 
                        const std::vector<int> src_dims,
                        const std::vector<int> dst_dims,
                        const std::vector<int> src_byte_stride,
        ...
        cpy_continue(
            static_cast<const char*>(src_buf.ptr), 
            static_cast<char*>(dst_buf.ptr),
            src_dims,
            dst_dims,
            src_byte_stride,
            dst_byte_stride,
    ...
    py::arg("src_dims"),
    py::arg("dst_dims"),
    py::arg("src_byte_stride"),
    py::arg("dst_byte_stride"),
    py::arg("src_dt"),
    py::arg("dst_dt")
);
}
