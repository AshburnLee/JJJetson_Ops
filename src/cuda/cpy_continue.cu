...
// 即当前布局下的 字节 stride
static int add_padding_to_storage(int ne0, int ne1, int ne2, int ne3, int nb0, int nb1, int nb2,
                                  int nb3, size_t elem_size) {
    const int last_off = (ne0 - 1) * nb0 + (ne1 - 1) * nb1 + (ne2 - 1) * nb2 + (ne3 - 1) * nb3;
...
cpy_continue_kernel(const char *src, char *dst, const int ne, const int ne0_0, const int ne0_1,
                    const int ne0_2, const int ne0_3, const int nb0_0, const int nb0_1,
                    const int nb0_2, const int nb0_3, const int ne1_0, const int ne1_1,
                    const int ne1_2, const int ne1_3, const int nb1_0, const int nb1_1,
                    const int nb1_2, const int nb1_3) {
...
template <typename src_t, typename dst_t>
static void
cpy_conitune_impl(const char *csrc, char *cdst,
    ...
    const int src_w_padding = add_padding_to_storage(ne0_0, ne0_1, ne0_2, ne0_3, nb0_0, nb0_1,
                                                     nb0_2, nb0_3, sizeof(src_t));
    const int dst_w_padding = add_padding_to_storage(ne1_0, ne1_1, ne1_2, ne1_3, nb1_0, nb1_1,
                                                     nb1_2, nb1_3, sizeof(dst_t));
    ...
    std::printf("Kernel launch config: block=(%lld,1,1), grid=(%lld,1,1)\n",
                static_cast<long long>(CUDA_CPY_BLOCK_SIZE), static_cast<long long>(num_blocks));
    std::fflush(stdout);
#endif
    cpy_continue_kernel<cuda_cast_t<src_t, dst_t>><<<num_blocks, CUDA_CPY_BLOCK_SIZE, 0, stream>>>(
        reinterpret_cast<const char *>(d_src), reinterpret_cast<char *>(d_dst), n_elem, ne0_0,
        ne0_1, ne0_2, ne0_3, nb0_0, nb0_1, nb0_2, nb0_3, ne1_0, ne1_1, ne1_2, ne1_3, nb1_0, nb1_1,
        nb1_2, nb1_3);
    LAUNCH_CHECK();
...
extern "C" void cpy_continue(const char *src, char *dst, const std::vector<int> &src_dims,
                             const std::vector<int> &dst_dims, const std::vector<int> &src_stride,
                             const std::vector<int> &dst_stride, data_type src_dt,
                             data_type dst_dt) {
    if (src_dt == data_type::DT_F32 && dst_dt == data_type::DT_F32) {
        ...
        //     cpy_conitune_impl<nv_bfloat16, nv_bfloat16>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // } else if (src_dt == data_type::DT_F32 && dst_dt == data_type::DT_BF16) {
        //     cpy_conitune_impl<float, nv_bfloat16>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // } else if (src_dt == data_type::DT_I32 && dst_dt == data_type::DT_BF16) {
        //     cpy_conitune_impl<int32_t, nv_bfloat16>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // } else if (src_dt == data_type::DT_F16 && dst_dt == data_type::DT_BF16) {
        //     cpy_conitune_impl<half, nv_bfloat16>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // } else if (src_dt == data_type::DT_BF16 && dst_dt == data_type::DT_F32) {
        //     cpy_conitune_impl<nv_bfloat16, float>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // } else if(src_dt == data_type::DT_BF16 && dst_dt == data_type::DT_F16) {
        //     cpy_conitune_impl<nv_bfloat16, half>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
        // }else if (src_dt == data_type::DT_BF16 && dst_dt == data_type::DT_I32) {
        //     cpy_conitune_impl<nv_bfloat16, int32_t>(src, dst,
        //     src_dims,dst_dims,src_stride,dst_stride);
