...
__global__ void quantize_q8_1_kernel(const float *__restrict__ x, // 输入浮点数组
    ...
    // grid
    // 的y方向有4个block，每个block有128个线程，每个thread负责4个float，正好覆盖输入变化最快的维度
    // 2048。
    ...
    float4 xi = (i0 < ne0_0) ? x4[(i3 * s0_3 + i2 * s0_2 + i1 * s0_1 + i0) / 4]
...
// 给出thread 和 输出的映射
/*
|线程索引 (`threadIdx.x`)| 输入索引 (`i0`)|输出索引 (qblock 索引`ib`, qblock中qs索引`iqs`)|存储位置
(`y[ib].qs`)|存储内容 (`char4`)|`ds4`存储位置索引`ds4_id`|`ds4` 存储|
|---|---|---|---|---|---|---|
...
extern "C" void q8_1(float *input, uint8_t *output, std::vector<int> &input_dims) {
    ...
    std::fflush(stdout);
#endif
    quantize_q8_1_kernel<<<blocks, threads, 0, stream>>>(d_x, d_y, ne0_0, s0_1, s0_2, s0_3, ne0_0,
                                                         ne0_1, ne0_2);
    LAUNCH_CHECK();
...
// 在调用 q8_1 之后，解析 output，将uint8_t的output parse得到 unit8_t 的 qs 和 16位的 scale 和
// 16位的 sum 只不过，用 fp32 来存储 scale 和 sum，因为后续的计算需要用到 float32 的精度
extern "C" void parse_q8_1_output(const uint8_t *input, // output from q8_1
