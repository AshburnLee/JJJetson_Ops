...
extern "C" void moe_dispatch_launch_cuda(cudaStream_t stream,
                                         const float* d_x,
...
static __global__ void moe_swiglu_silu_mul_kernel(const float* gate,
                                                  const float* up,
                                                  float* mid,
                                                  int n_elem) {
...
static void moe_swiglu_experts_forward_cublas_device(cudaStream_t stream,
    ...
    moe_dispatch_launch_cuda(stream,
                           d_x,
                           d_expert_ids,
                           num_tokens,
                           top_k,
    ...
    CUDA_CHECK(cudaMemcpyAsync(h_off_host,
                               d_off,
    ...
    for (int e = 0; e < num_experts; ++e) {
        ...
        // op(B) : k x n
        // C     : m x n
        // C 物理上的 row-major 3x2 在API看来是 col-major的 2x3 !!! 这是理解 row-major 数据和col-major API的关键
        // 对于 md 中 case e=1 时的参数列表：
        // 2x4, 4x3 = 2x3, 故OP(A) 是Wg^T, OP(B) 是Xe，输出在row-major时 是 3x2 的,所以
        // 是C（Ge） 决定了 A 是权值， B 是x !!!
    ...
    CUDA_CHECK(cudaMemsetAsync(d_y, 0, static_cast<size_t>(num_tokens) * H * sizeof(float), stream));
    moe_combine_launch_cuda(
        stream, d_expert_out, d_src_t, d_src_k, d_route_weights, d_y, R, H, top_k);
}
...
extern "C" void moe_swiglu_experts_forward(const float* x_host,
    ...
    int *d_ids = nullptr, *d_src_t = nullptr, *d_src_k = nullptr, *d_off = nullptr, *d_counts = nullptr,
        *d_next = nullptr;
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_ids, expert_ids_host, rk_sz * sizeof(int), cudaMemcpyHostToDevice, s));
    CUDA_CHECK(
        cudaMemcpyAsync(d_rw, route_weights_host, rk_sz * sizeof(float), cudaMemcpyHostToDevice, s));
    CUDA_CHECK(cudaMemcpyAsync(d_wg, w_gate_host, w_gu_sz * sizeof(float), cudaMemcpyHostToDevice, s));
    CUDA_CHECK(cudaMemcpyAsync(d_wu, w_up_host, w_gu_sz * sizeof(float), cudaMemcpyHostToDevice, s));
    CUDA_CHECK(cudaMemcpyAsync(d_wd, w_down_host, w_d_sz * sizeof(float), cudaMemcpyHostToDevice, s));

    moe_swiglu_experts_forward_cublas_device(s,
                                             handle,
                                             d_x,
                                             d_ids,
                                             d_rw,
