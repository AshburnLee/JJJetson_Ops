...
template <typename T>
__device__ inline int find_total_elts_less_than_target(T const* sorted_indices,
                                                       int arr_length,
                                                       T target) {
    int low = 0;
...
__global__ void moe_dispatch_sota_expand_rows_kernel(const float* d_x,
                                                     float* d_permuted_x,
...
extern "C" void moe_dispatch_sota_launch_cuda(cudaStream_t stream,
                                              const float* d_x,
    ...
    moe_dispatch_sota_fill_arange_kernel<<<blocks_fill, threads_fill, 0, stream>>>(
        d_arange_buf, num_routes);
    LAUNCH_CHECK();
...
extern "C" void moe_dispatch_sota(const float* x_host,
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_ids, expert_ids_host, route_int_bytes, cudaMemcpyHostToDevice,
                               stream));
    ...
    CUDA_CHECK(
        cudaMemcpyAsync(source_token_host, d_src_t, route_int_bytes, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(
