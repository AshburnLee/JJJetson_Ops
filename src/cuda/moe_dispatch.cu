...
static __global__ void moe_dispatch_hist_kernel(const int* expert_ids,
                                                int num_routes,
                                                int num_experts,
                                                int* counts) {
...
static __global__ void moe_dispatch_exclusive_prefix_kernel(const int* counts,
                                                            int num_experts,
                                                            int* offsets) {
...
static __global__ void moe_dispatch_init_next_kernel(const int* offsets, 
                                                     int num_experts, 
                                                     int* next_slot) {
...
extern "C" void moe_dispatch_launch_cuda(cudaStream_t stream,
                                           ...
                                           int* d_next_slot) {
    const int num_routes = num_tokens * top_k;
    CUDA_CHECK(cudaMemsetAsync(d_counts, 0, static_cast<size_t>(num_experts) * sizeof(int), stream));

    const int threads = 256;
    const int blocks_hist = (num_routes + threads - 1) / threads;
    moe_dispatch_hist_kernel<<<blocks_hist, threads, 0, stream>>>(
        d_expert_ids, num_routes, num_experts, d_counts);
    LAUNCH_CHECK();
    
    // 1 个thread？
    moe_dispatch_exclusive_prefix_kernel<<<1, 1, 0, stream>>>(d_counts, num_experts, d_expert_offsets);
    LAUNCH_CHECK();
    ...
    moe_dispatch_scatter_kernel<<<blocks_hist, threads, 0, stream>>>(d_x,
                                                                     d_expert_ids,
                                                                     num_tokens,
                                                                     top_k,
                                                                     hidden_size,
...
extern "C" void moe_dispatch(const float* x_host,
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_x, x_host, x_size * sizeof(float), cudaMemcpyHostToDevice, stream));
    ...
    CUDA_CHECK(cudaMemcpyAsync(permuted_x_host, d_perm, perm_size * sizeof(float), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(source_token_host, d_src_t, id_size, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(source_k_host, d_src_k, id_size, cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(expert_offsets_host,
                               d_off,
