...
__global__ void __launch_bounds__(256, 4)
    fa_kernel_one_pass_parallel_tc_true(const half *__restrict__ Q, const half *__restrict__ K,
    ...
    for (int tile_id = 0; tile_id < LOOP_KV; ++tile_id) {
        ...
        for (int i = tid; i < K_VEC; i += block_threads) {
            const int row = i / (HEAD_DIM / 2);
            const int j2 = i % (HEAD_DIM / 2);
            const half2 *src_h2 = reinterpret_cast<const half2 *>(k_tile_base + row * HEAD_DIM);
            reinterpret_cast<half2 *>(&kv_shared[row][0])[j2] = src_h2[j2];
        }
        __syncthreads();

        // WMMA 第一段：S = Q * K^T。K 在 kv_shared 为row-major[seq][d]；B 用 col_major
        // 从同一缓冲加载， 避免了转置。输出 32×32 half，warp 0..3 负责 2×2 个 16×16 象限
        /*  s_scores:
         *             列 0..15     列 16..31
         *           +-------------+-------------+
        ...
        for (int i = tid; i < K_VEC; i += block_threads) {
            const int row = i / (HEAD_DIM / 2);
            const int j2 = i % (HEAD_DIM / 2);
            const half2 *src_h2 = reinterpret_cast<const half2 *>(v_tile_base + row * HEAD_DIM);
            reinterpret_cast<half2 *>(&kv_shared[row][0])[j2] = src_h2[j2];
        }
        __syncthreads();

        // WMMA #2：O_tile = PV，累加到 pv_acc（逻辑 32×128
        // FP32）。P=s_scores（32×32），V=kv_shared（32×128）
        /*
         ...
         *         +--------+--------+--------+ ... +--------+
         *
         *  内层 k_step=0,1：沿 KV token 维 32 = 2×16 顺序累加（每 (m_tile,n_tile) 共 2 次
         *mma，非并行）
         */
        for (int n_tile = warp_id; n_tile < N_TILES /*=8*/; n_tile += num_warps /*=8*/) {
            ...
            for (int m_tile = 0; m_tile < WMMA_ROWS / WMMA_M /*=32/16=2*/; ++m_tile) {
                ...
                for (int k_step = 0; k_step < KV_TOKEN_TILE / WMMA_K /*32/16=2*/; ++k_step) {
                    ...
                    load_matrix_sync(b_frag, &kv_shared[k_step * WMMA_K][n_tile * WMMA_N],
                                     V_STRIDE);
...
extern "C" void fa_one_pass_parallel_tc_true(const uint16_t *q_host, const uint16_t *k_host,
    ...
    if (static_shmem > static_cast<size_t>(kDefaultShmemPerBlock)) {
        ...
        if (excess > 0) {
            ...
            CUDA_CHECK(cudaFuncSetAttribute((void *)fa_kernel_one_pass_parallel_tc_true,
    ...
    fa_kernel_one_pass_parallel_tc_true<<<blocks, threads, 0, stream>>>(d_q, d_k, d_v, d_dst,
                                                                        scale);
    LAUNCH_CHECK();
