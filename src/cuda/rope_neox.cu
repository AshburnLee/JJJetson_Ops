...
rope_neox_kernel(const float *x,   // 对 x 进行 RoPE 操作
                 float *dst,       // 将结果写入 dst
...
extern "C" void rope(float *input, int *pos, float *output, std::vector<int> &input_dims) {
    ...
    std::fflush(stdout);
#endif
    rope_neox_kernel<true><<<blocks, threads, 0, stream>>>(d_x, d_y, ne0_0, ne0_1, s0_1, s0_2,
                                                           n_dims, d_pos, freq_scale, ext_factor,
