...
static __global__ void moe_combine_kernel(const float *expert_out, const int *source_token,
                                          const int *source_k, const float *route_weights, float *y,
...
extern "C" void moe_combine_launch_cuda(cudaStream_t stream, const float *d_expert_out,
    ...
    moe_combine_kernel<<<blocks, threads, 0, stream>>>(d_expert_out, d_source_token, d_source_k,
                                                       d_route_weights, d_y, num_routes,
                                                       hidden_size, top_k);
    LAUNCH_CHECK();
}
...
extern "C" void moe_combine(const float *expert_out_host, const int *source_token_host,
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_out, expert_out_host, out_size * sizeof(float),
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(
        cudaMemcpyAsync(d_y, y_host, y_size * sizeof(float), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_rw, route_weights_host, rw_size, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(
        cudaMemcpyAsync(d_src_t, source_token_host, meta_size, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_src_k, source_k_host, meta_size, cudaMemcpyHostToDevice, stream));

    moe_combine_launch_cuda(stream, d_out, d_src_t, d_src_k, d_rw, d_y, num_routes, hidden_size,
                            top_k);

    CUDA_CHECK(
        cudaMemcpyAsync(y_host, d_y, y_size * sizeof(float), cudaMemcpyDeviceToHost, stream));
