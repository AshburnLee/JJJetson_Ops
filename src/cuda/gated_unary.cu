...
ne_stride1 = 6144              src_1->nb[1]/sizeof(T) =
6144*4/4=6144（这是连续的情况下），即第二个维度元素的 stride num_block = 312 num_thread = 256
*/
template <float (*op)(float), typename T>
static __global__ void unary_gated_op_kernel(const T *src, const T *gate, T *dst,
                                             const int64_t dst_elem, const int64_t ne0,
                                             const int64_t ne1, const int64_t ne_stride0,
    ...
    // 答：因为有 ne_stride0 存在，得到的 j0 不是连续的：j =
    // (0,1,2,3,8,9,10,11,16,17,18,19)，中间跳过的就是padding值 故 dst 中只有真实值
    if (g_id >= dst_elem) {
        return;
    }
...
template <float (*op)(float), typename T>
static void unary_gated(const T *src0, const T *src1, T *dst, const std::vector<int> &dst_dims,
                        const std::vector<int> &src0_nb, const std::vector<int> &src1_nb) {
    ...
    int64_t ne_stride0 =
        src0_nb[1] / sizeof(T); // 这个维度 包括padding，将Byte stride 转换成 元素个数后的 stride
    int64_t ne_stride1 =
        src1_nb[1] / sizeof(T); // 这个维度 包括padding，将Byte stride 转换成 元素个数后的 stride
    ...
    CUDA_CHECK(
        cudaMemcpyAsync(d_src0, src0, src0_elem * sizeof(T), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(
        cudaMemcpyAsync(d_src1, src1, src1_elem * sizeof(T), cudaMemcpyHostToDevice, stream));
...
extern "C" void relu_gated(const float *src0, const float *src1, float *dst,
                           const std::vector<int> &dst_dims, const std::vector<int> &src0_nb,
