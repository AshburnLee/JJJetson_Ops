...
def test_prefill():
    ...
    rope_global_cache_me.init_cossin_cache(max_len, ROPE_D, ROPE_FREQ_BASE)
    try:
        rope_global_cache_me.RoPE(input_np, pos_np, output_np, dims)
    finally:
        rope_global_cache_me.destroy_cossin_cache()
...
def test_decode():
    ...
    np.random.seed(24)
    # decode 1 token : (ROPE_D, ROPE_H, ROPE_T=1, ROEP_B)
